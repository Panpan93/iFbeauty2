...
- (void)loadDataBegin {
    ...
    if (loadingMore == NO) /**没有在加载**/{
        ...
        [self.matchTV.tableFooterView addSubview:_tableFooterAI];
...
- (void)beforeLoadEnd {
    UILabel *label = (UILabel *)[self.matchTV.tableFooterView viewWithTag:9001];
...
- (void)loadDataEnd {
    self.matchTV.tableFooterView = [[UIView alloc] init];
    loadingMore = NO;
    
}

- (void)createTableFooter {
    UIView *tableFooterView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, self.matchTV.bounds.size.width, 40.0f)];
    ...
    self.matchTV.tableFooterView = tableFooterView;
}
